...
module thalaswap_math::package {
    ...
    //                                                                        //
    const DEPLOYER_ADDRESS: address = @thalaswap_math_deployer;               //
    const RESOURCE_ACCOUNT_SEED: vector<u8> = b"thalaswap_math";              //
    const RESOURCE_ACCOUNT_ADDRESS: address = @thalaswap_math;                //
    //                                                                        //
    ...
    #[test(thalaswap_math = @thalaswap_math)]
    #[expected_failure(abort_code = ERR_PACKAGE_UNAUTHORIZED)]
    fun publish_package_unauthorized_err(thalaswap_math: &signer) acquires ResourceSignerCapability {
        // setup the manager & package resource
        thala_manager::manager::initialize_for_test(signer::address_of(thalaswap_math));
        init_for_test();
